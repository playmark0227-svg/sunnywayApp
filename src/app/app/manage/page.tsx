  ...
  return (
    <div className="fade">
      <header className="sticky top-0 z-20 flex items-center justify-between bg-canvas/80 px-5 pb-3 backdrop-blur-xl" style={{ paddingTop: "max(0.9rem,env(safe-area-inset-top))" }}>
        ...
        <Link href="/app/history" className="rounded-full px-3 py-2 text-sm font-medium text-sunny-600 hover:bg-sunny-50">応募履歴</Link>
      </header>

      <div className="px-5 pt-1"><div className="flex rounded-full bg-ink/5 p-1">
        {TABS.map(([k, label]) => (
          <Link key={k} href={`/app/manage?tab=${k}`} className={`flex-1 rounded-full py-2 text-center text-sm font-semibold transition ${tab === k ? "bg-surface text-ink shadow-soft" : "text-ink/50"}`}>{label}{counts[k as keyof typeof counts] ? ` ${counts[k as keyof typeof counts]}` : ""}</Link>
        ))}
      </div></div>
